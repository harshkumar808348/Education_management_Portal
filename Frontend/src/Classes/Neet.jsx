...
const Neet = () => {
  ...
  useEffect(() => {
    const fetchNeetData = async () => {
      try {
        const response = await fetch('http://localhost:3000/educational-services/neet-exams');
        if (!response.ok) {
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          ...
          <p className="text-gray-600 mt-1">NEET Faculty Members</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
          {neetData && neetData.length > 0 ? (
            neetData.map((teacher) => (
              <div
